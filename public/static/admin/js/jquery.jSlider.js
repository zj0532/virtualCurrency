...
(function(factory) {
	...
	$.fn.extend({
		...
		'sliderInit': function(options) {
			...
			/* if found selected item, store the foot_menu & update class */
			...
			this.data('total', total)
				...
				.data('map-foot_menu', sliderMap.length)
		...
		, 'sliderStop': function() {
			var index = this.data('map-foot_menu');
			clearInterval(sliderMap[index]['sliderInterval']);
		...
		, 'sliderShowItems': function() {
			var $slider = this
				, current = $slider.data('current')
				, index =  $slider.data('map-foot_menu')
